const fs=require('fs');
const assert=(ok,msg)=>{if(!ok){console.error('Trip UI v1.22.10 audit failed:',msg);process.exit(1)}};
...
assert(site.includes('/trip-workspace-refine-v12210.js?v=12210'),'v1.22.10 refinement must be injected into HTML');
assert(site.includes("VERSION='1.22.10'"),'site wrapper must report v1.22.10');
assert(pkg.version==='1.22.10','package version must be 1.22.10');
assert(pkg.main==='site-v12210.js'&&pkg.scripts.start==='node site-v12210.js','package must start the v1.22.10 wrapper');
console.log('Trip UI v1.22.10 audit passed: PDF button ownership, top trip editing, redundant Park Order removal, and hidden booklet source rendering are configured.');
