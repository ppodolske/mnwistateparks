...
for(const token of ['tripChooserModal','trip-choice','tripAddParkDirect','tripParkSearch','Add another stop','shareTripBtn','Copy share link','shared','importSharedTrip','Save this trip','tripRouteOverview','DAY-BY-DAY','Route overview','Open in Google Maps','straight-line distance','placeQuery','State Park','dayPlanHelp','PLAN YOUR DAYS','saveDayAssignmentsBtn','Save day changes','Day assignments saved'])if(!plannerJS.includes(token))throw new Error(`planner-polish.js missing ${token}`);
...
const injectedTrip=injectPlanner(trip);if(!injectedTrip.includes('tripAddParkDirect')||!injectedTrip.includes('shareTripBtn')||!injectedTrip.includes('tripRouteOverview')||!injectedTrip.includes('saveDayAssignmentsBtn'))throw new Error('Planner/route/day polish not injected into trip page response');
