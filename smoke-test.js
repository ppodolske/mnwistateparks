...
for(const token of ['tripChooserModal','trip-choice','tripAddParkDirect','tripParkSearch','Add another stop','shareTripBtn','Copy share link','shared','importSharedTrip','Save this trip','tripRouteOverview','DAY-BY-DAY','Route overview','placeQuery','State Park','dayPlanHelp','PLAN YOUR DAYS','saveDayAssignmentsBtn','Save day changes','Day assignments saved','Open park in Google Maps','Open day route in Google Maps','exportTrip','Export trip PDF','Save as PDF','Generated from the MN & WI State Parks trip planner','Critical factor:'])if(!plannerJS.includes(token))throw new Error(`planner-polish.js missing ${token}`);
if(plannerJS.includes("query='+encodeURIComponent(stops[0].lat+','+stops[0].lng)"))throw new Error('Single-stop Google Maps links still use raw coordinates');
if(plannerJS.includes("origin='+encodeURIComponent(origin.lat+','+origin.lng)"))throw new Error('Multi-stop Google Maps links still use raw coordinates');
...
const injectedTrip=injectPlanner(trip);if(!injectedTrip.includes('tripAddParkDirect')||!injectedTrip.includes('shareTripBtn')||!injectedTrip.includes('tripRouteOverview')||!injectedTrip.includes('saveDayAssignmentsBtn')||!injectedTrip.includes('Export trip PDF'))throw new Error('Planner/route/day/PDF polish not injected into trip page response');
...
console.log('Smoke tests passed: polished trip PDF export, named Google Maps park/day routes, explicit day save, shareable trips, planner polish, and 116/116 map.');
