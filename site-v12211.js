...
const previous=require('./site-v12210.js');
const VERSION='1.22.11';
const PORT=process.env.PORT||3000;
const CLEANUP_FILE=path.join(__dirname,'trip-home-cleanup-v12211.js');
const cleanupJS=fs.readFileSync(CLEANUP_FILE,'utf8');
...
function createServer(){
  ...
  server.on('request',(req,res)=>{
    ...
    if(url.pathname==='/trip-home-cleanup-v12211.js'){
      res.writeHead(200,{'content-type':'application/javascript; charset=utf-8','cache-control':'no-cache'});
      return res.end(cleanupJS);
    }
    if(url.pathname==='/health'){
      const coords=previous.loadCoords();
      res.writeHead(200,{'content-type':'application/json','cache-control':'no-store'});
      return res.end(JSON.stringify({ok:true,version:VERSION,architecture:'current-runtime-v12211-redundancy-cleanup',parks:previous.parks.length,collections:previous.COLLECTIONS.length,parkReferenceRecords:Object.keys(previous.parkDetails||{}).length,mapCoordinates:coords?Object.keys(coords.parks||{}).length:0,features:['yellowstone-brand-system','home-hero-count-removed','legacy-trip-details-removed','top-trip-editor','redundant-park-order-hidden','booklet-source-hidden','owned-pdf-download-action','detailed-and-condensed-export-controls','isolated-iframe-pdf-rendering','download-only-pdf-export','export-state-synchronization','material-park-favicon','preston-run-footer-link']}));
    }
    ...
    res.writeHead=function(statusCode,statusMessage,headers){
      ...
      if(suppliedHeaders){
        ...
        if(type.includes('text/html')){
          ...
          suppliedHeaders['x-parks-release']='v1.22.11';
        }
      }
    ...
    res.end=function(body,...args){
      if(body!=null){
        ...
        if(looksLikeHtml){
          const marker='<meta name="parks-release-version" content="1.22.11">';
          const runtime='<script src="/trip-home-cleanup-v12211.js?v=12211" defer></script>';
          html=html
            .replace(/<meta name="parks-release-version"[^>]*>/gi,'')
            .replace(/<script[^>]*trip-home-cleanup-v12211\.js[^>]*><\/script>/gi,'');
...
if(require.main===module)createServer().listen(PORT,'0.0.0.0',()=>console.log(`State Parks v${VERSION} on ${PORT} (redundant home and trip UI removed)`));
module.exports={...previous,VERSION,cleanupJS,createServer};
