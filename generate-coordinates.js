...
const MANUAL={
  'cross-plains':{lat:43.085592,lng:-89.612215,source:'Wisconsin DNR location page',sourceName:'Cross Plains State Park'},
  'devil-s-lake':{lat:43.4284,lng:-89.7322,source:'Wisconsin DNR location page',sourceName:"Devil's Lake State Park"},
  'interstate-park':{lat:45.395413,lng:-92.649259,source:'Wisconsin DNR location page',sourceName:'Interstate State Park'},
  'kohler-andrae':{lat:43.66843,lng:-87.717287,source:'Wisconsin DNR location page',sourceName:'Kohler-Andrae State Park'},
  'mill-bluff':{lat:43.9411,lng:-90.3189,source:'Wisconsin DNR address / validated geocode',sourceName:'Mill Bluff State Park'}
};
...
const aliases={'father-hennepin':'father hennepin','grand-portage':'grand portage','john-a-latsch':'john a latsch','lake-louise':'lake louise','minneopa':'minneopa','old-mill':'old mill','st-croix':'st croix','governor-dodge':'governor dodge','heritage-hill':'heritage hill','new-glarus-woods':'new glarus woods','nelson-dewey':'nelson dewey','tower-hill':'tower hill','wyalusing':'wyalusing'};
...
async function query(url,params){const u=new URL(url);for(const [k,v] of Object.entries(params))u.searchParams.set(k,String(v));const r=await fetch(u,{headers:{'user-agent':'mn-wi-state-parks-coordinate-build/1.2'}});if(!r.ok)throw new Error(`GIS request failed ${r.status} ${u}`);const j=await r.json();if(j.error)throw new Error(`GIS error ${JSON.stringify(j.error)}`);return j.features||[]}
(async()=>{
 ...
 for(const p of parks){if(MANUAL[p.slug])continue;const src=p.state==='MN'?mn:wiAttrs,m=bestMatch(p,src);if(!m){unmatched.push({slug:p.slug,name:p.name,state:p.state});continue}matches.set(p.slug,m)}
 ...
 const out={};
 for(const p of parks){if(MANUAL[p.slug]){out[p.slug]={...MANUAL[p.slug],matchScore:100};continue}const m=matches.get(p.slug);if(!m)continue;let c=null,feature=m.feature;if(p.state==='MN'){const g=feature.geometry||{};c=Number.isFinite(g.y)&&Number.isFinite(g.x)?{lat:g.y,lng:g.x}:largestRingCentroid(g)}else{feature=wiById.get(m.feature.attributes.OBJECTID);c=largestRingCentroid(feature&&feature.geometry)}if(!c||!Number.isFinite(c.lat)||!Number.isFinite(c.lng)){unmatched.push({slug:p.slug,name:p.name,state:p.state,reason:'no geometry',matched:m.value});continue}out[p.slug]={lat:+c.lat.toFixed(6),lng:+c.lng.toFixed(6),source:p.state==='MN'?'Minnesota DNR':'Wisconsin DNR GIS',sourceName:m.value,matchScore:m.score};}
 ...
 fs.writeFileSync(OUT,JSON.stringify({generatedAt:new Date().toISOString(),count:parks.length,sources:{MN:MN_URL,WI:WI_URL,manual:Object.keys(MANUAL)},parks:out},null,2));
