...
function createServer(){
  return http.createServer((req,res)=>{
    ...
    if(url.pathname==='/health'){
      ...
      return res.end(JSON.stringify({ok:true,version:VERSION,architecture:'current-runtime',parks:site.parks.length,collections:site.COLLECTIONS.length,parkReferenceRecords:Object.keys(parkDetails).length,features:['saved-parks','compare','trip-collections','trip-data-model-v2','trip-itinerary-model','itinerary-summary-ui','trip-booklet-overview','trip-booklet-daily-pages','booklet-pagination-engine','booklet-no-split-stop-cards','booklet-a4-letter-metrics','booklet-handwriting-space','booklet-park-tags','booklet-critical-factors','editable-trip-start-location','editable-trip-end-location','day-date-calculation','day-start-location','day-overnight-location','trip-route-links','park-reference-data','park-reference-coverage-116','trip-day-planner','trip-stop-notes','trip-stop-details-editor','trip-map','compact-trip-pdf','shareable-trips','day-route-overview','name-based-map-routing','explicit-day-save','auto-park-address','google-maps-place-link','camping-details','campground-name','campground-loop','campsite-number','reservation-details','check-in-out','trip-start-end-dates','trip-locations','emergency-contact','lodging-notes','resupply-notes','external-planner-scripts','no-split-stop-cards','photo-free-pdf'],mapCoordinates:coords?Object.keys(coords.parks).length:0}));
    }
...
module.exports={...site,VERSION,tripModelJS,tripItineraryJS,tripItineraryUIJS,tripBookletOverviewJS,tripBookletDaysJS,tripBookletPaginationJS,plannerJS,tripDetailsJS,parkDetailsJS,parkDetailsExtraJS,parkAddressesCurrentJS,parkDetails,injectAll,createServer};
