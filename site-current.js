...
function createServer(){
  return http.createServer((req,res)=>{
    ...
    if(url.pathname==='/health'){
      ...
      return res.end(JSON.stringify({ok:true,version:VERSION,architecture:'current-runtime',parks:site.parks.length,collections:site.COLLECTIONS.length,parkReferenceRecords:Object.keys(parkDetails).length,features:['saved-parks','compare','trip-collections','trip-data-model-v2','trip-itinerary-model','itinerary-summary-ui','trip-booklet-overview','trip-booklet-daily-pages','trip-booklet-route-page','trip-booklet-reference-page','trip-booklet-notes-page','booklet-logistics-reference','booklet-trip-highlights','booklet-handwriting-space','booklet-numbered-stop-map','booklet-route-order','booklet-pagination-engine','booklet-no-split-stop-cards','booklet-a4-letter-metrics','booklet-pdf-export','booklet-export-mode','compact-export-mode','booklet-paper-selector','booklet-render-repair','booklet-physical-write-lines','booklet-section-owned-pagination','booklet-park-tags','booklet-critical-factors','editable-trip-start-location','editable-trip-end-location','day-date-calculation','day-start-location','day-overnight-location','trip-route-links','park-reference-data','park-reference-coverage-116','trip-day-planner','trip-stop-notes','trip-stop-details-editor','trip-map','shareable-trips','day-route-overview','name-based-map-routing','explicit-day-save','auto-park-address','google-maps-place-link','camping-details','campground-name','campground-loop','campsite-number','reservation-details','check-in-out','trip-start-end-dates','trip-locations','emergency-contact','lodging-notes','resupply-notes','external-planner-scripts','no-split-stop-cards','photo-free-pdf'],mapCoordinates:coords?Object.keys(coords.parks).length:0}));
    }
...
module.exports={...site,VERSION,tripModelJS,tripItineraryJS,tripItineraryUIJS,tripBookletOverviewJS,tripBookletDaysJS,tripBookletRouteJS,tripBookletReferenceJS,tripBookletNotesJS,tripBookletPaginationJS,tripBookletExportJS,plannerJS,tripDetailsJS,parkDetailsJS,parkDetailsExtraJS,parkAddressesCurrentJS,parkDetails,injectAll,createServer};
