...
const previous=require('./site-v1228.js');
const VERSION='1.22.9';
const PORT=process.env.PORT||3000;
const UI_FILE=path.join(__dirname,'trip-workspace-ui-v1229.js');
const tripWorkspaceUIJS=fs.readFileSync(UI_FILE,'utf8');
  ...
  server.on('request',(req,res)=>{
    ...
    res.writeHead=function(statusCode,statusMessage,headers){
      ...
      if(suppliedHeaders){
        ...
        if(type.includes('text/html')){
          ...
          suppliedHeaders['x-parks-release']='v1.22.9';
        }
      }
    ...
    res.end=function(body,...args){
      if(body!=null){
        ...
        if(looksLikeHtml){
          const marker='<meta name="parks-release-version" content="1.22.9">';
          const runtime='<script src="/trip-workspace-ui-v1229.js?v=1229" defer></script>';
          html=html
            .replace(/<meta name="parks-release-version"[^>]*>/gi,'')
            .replace(/<script[^>]*trip-workspace-ui-v1229\.js[^>]*><\/script>/gi,'');
...
if(require.main===module)createServer().listen(PORT,'0.0.0.0',()=>console.log(`State Parks v${VERSION} on ${PORT} (readable trip workspace and restored export controls)`));
module.exports={...previous,VERSION,tripWorkspaceUIJS,createServer};
